  ...
  <div>
    ...
    <div v-if="!loading">
      <el-form status-icon label-width="80px" :model="form" :rules="rules" ref="form">
        ...
        <!--               <el-checkbox v-for="(item, index) in recleaningList" :label="item.name" :key="index">{{
            item.name }}-->
        <!--               </el-checkbox>-->
        <!--             </el-checkbox-group>-->
        <!--           </div>-->
...
export default {
  ...
  data () {
    return {
      ...
      form: {},
      rules: {
        schoolUserName: [{ required: true, trigger: 'blur', validator: validateUsername }],
        enrollmentYear: [
          {
  ...
  methods: {
    ...
    removeClassInfo (studentId) {
      ...
      operationTip({
        ...
        removeStudentToClass(studentId).then(res => {
          if (res.errorCode === 200) {
            successTip();
            that.params.gradeId = '';
    ...
    operationData () {
      ...
      that.$refs['form'].validate(valid => {
        if (valid) {
          that.loading = true;
          updateStudentInfo(that.form).then(res => {
            if (res.errorCode === 200) {
              successTip();
              that.loading = false;
              that.form = {};
